...
class TestNav5Creation:
    ...
    def test_clone_upload_voice(self, active_window):
        ...
        assert result == True
        # 后置：点击 Continue 进入信息填写页，再点击 Back 返回上传页，点击 delete.png 删除已上传音频
    ...
    def test_clone_delete_voice(self, active_window):
        """
        进入克隆页面，上传音频后点击delete图片删除，continue图片不存在则用例成功
        """
        nav5_page = VoicewaveVoiceCreationPage(active_window)
        nav5_page.nav5_voice_creation()
        # 克隆页面，clone按钮控件元素，存在则点击，不存在则跳过
    ...
    def test_clone_click_continue(self, active_window):
        ...
        assert result == True
        # 后置：点击 Back 返回上传页，点击 delete.png 删除已上传音频
        nav5_page.click_back_btn()
        nav5_page.find_element_and_click("nav5_voice_vreation/delete.png")

    @allure.story("克隆点击Back按钮")
    @allure.title("克隆页面-点击Back按钮-返回上传音频页-delete图片正常出现并删除")
    def test_clone_click_back(self, active_window):
        """
        进入克隆页面，上传音频并点击Continue后，点击Back按钮返回，delete图片出现则用例成功，并点击delete删除已上传音频
        """
        nav5_page = VoicewaveVoiceCreationPage(active_window)
        nav5_page.nav5_voice_creation()
        # 克隆页面，clone按钮控件元素，存在则点击，不存在则跳过
        ...
        # 点击 Back 按钮
        control_raise(
            active_window.CustomControl(ClassName='QStackedWidget', Depth=4).ButtonControl(Name='Back', ClassName='CustomBtn', Depth=7),
            "克隆页面-Back按钮")
        nav5_page.click_back_btn()
        # 检查 delete.png 是否出现
        result = nav5_page.find_element("nav5_voice_vreation/delete.png")
        assert result == True
        # 后置：点击delete图片删除已上传音频
        nav5_page.find_element_and_click("nav5_voice_vreation/delete.png")
    ...
    def test_clone_continue_public(self, active_window):
        """
        进入克隆页面，上传音频并点击Continue按钮，public图片出现则用例成功
        """
        nav5_page = VoicewaveVoiceCreationPage(active_window)
        nav5_page.nav5_voice_creation()
        # 克隆页面，clone按钮控件元素，存在则点击，不存在则跳过
    ...
    def test_clone_input_voice_name(self, active_window):
        """
        进入克隆页面，上传音频并点击Continue后，输入名称选择Anime标签上传图片，Create.png出现则用例成功
        """
        nav5_page = VoicewaveVoiceCreationPage(active_window)
        nav5_page.nav5_voice_creation()
        # 克隆页面，clone按钮控件元素，存在则点击，不存在则跳过
        ...
        nav5_page.find_control_and_input(desc_control, "Wait and Retry: Server glitches often resolve within a few seconds or minutes.Check Status Pages: Verify if the API provider is experiencing a widespread outage.Reduce Payload Size: Large inputs, long prompts,")
        # 点击 Upload image 按钮
        upload_img_btn = active_window.CustomControl(ClassName='QStackedWidget', Depth=4).GroupControl(ClassName='QWidget', foundIndex=5, Depth=7).ButtonControl(Name='Upload image', ClassName='CustomBtn', Depth=3)
    ...
    @pytest.mark.dependency(depends=["test_clone_input_voice_name"])
    ...
    def test_clone_enter_training_page(self, active_window):
        ...
        # 点击 Upload image 按钮
        element_raise("nav5_voice_vreation/upload_image.png", "克隆页面-Upload image按钮")
        nav5_page.find_element_and_click("nav5_voice_vreation/upload_image.png")
        # 在文件管理器中输入路径并打开文件
        file_path = project_root.getProjectRoot() + r"\resources\images\nav5_voice_vreation"
        nav5_page.input_file_path(file_path)
        nav5_page.open_file("p2.png")
        ...
        cancel_training_btn = active_window.CustomControl(ClassName='QStackedWidget', Depth=4).ButtonControl(Name='Cancel training task', ClassName='QPushButton', Depth=7)
        result = nav5_page.find_control(cancel_training_btn, 300)
        assert result == True
        # 后置：点击 Cancel training task 按钮和 Yes 按钮取消训练
        nav5_page.click(cancel_training_btn)
        yes_btn = active_window.GroupControl(ClassName='CancelTrainingConfirmDialog', Depth=1).ButtonControl(Name='Yes', ClassName='CustomBtn', Depth=1)
        nav5_page.click(yes_btn)
    ...
    def test_clone_input_name_select_anime(self, active_window):
        ...
        # 点击二级弹窗 Yes 按钮
        yes_btn = active_window.GroupControl(ClassName='CancelTrainingConfirmDialog', Depth=1).ButtonControl(Name='Yes', ClassName='CustomBtn', Depth=1)
        control_raise(yes_btn, "克隆页面-取消训练确认弹窗Yes按钮")
        nav5_page.click(yes_btn)
